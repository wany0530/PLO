...
#notice-wrapper #search-container {
	text-align: center;
	margin-bottom: 50px;
...
#notice-wrapper #search-container #search-box {
	display: inline-block;
...
#notice-wrapper #search-container #search-box > input {
	height: 19px;
	width: 380px;
	border: none;
	padding-left: 10px;
...
#notice-wrapper #search-container #search-box > button {
	width: 30px;
	height: 100%;
	background: #fff;
	border: none;
}
#notice-wrapper #search-container #search-box .fa-search {
	color: #A1A1A1;
}
#notice-wrapper #search-container #search {
	display: inline-block;
	margin-left: 20px;
}
#notice-wrapper #search-container #search button {
	color: #ff8533;
	font-size: 16px;
	background: #fff;
	border: none;
...
#notice-wrapper #sort-tab {
	margin-bottom: 30px;
}

#notice-wrapper #sort-tab li:after {
	content: '|';
	margin-left: 10px;
	color: #C1C1C1;
}

#notice-wrapper #sort-tab li {
	display: inline-block;
	margin-right: 10px;
}
#notice-wrapper #sort-tab button {
	border: none;
	background: #ffffff;
	font-size: 15px;
	outline: none;
...
<body>
	...
	<div id="notice-wrapper">
		...
		<div id="content-top">
			...
			<div id="link-btn">
				<ul>
					<li class="unSelect"><a href="${path}/notice">공지사항</a></li>
		...
		<div id="content-main">
			...
			<div id="search-container">
				<div id="search-box">
					...
					<input name="title" value="${param.title}" type="text" placeholder="사용TIP, 서비스문의, 재생목록"/>
					
				</div>
				<span id="search"><button>찾기</button></span>
			</div>
			<!-- 정렬 -->
			<div id="sort-tab">
				<ul class="fqa_code_grop">
				</ul>
			</div>
			</form>
			...
			<table>
				...
				<tbody>
					...
					<tr class="title" id="title${faq.f_no}">
						<td class="center">${faq.f_no}</td>
						<td class="pl-25" id="d_title${faq.f_no}">
							<span style="font-weight: bold;">${faq.fc_name}	&nbsp;&nbsp;&nbsp;</span>
							${faq.f_title}
						</td>
						<td class="center">
							<input type="button" value="⋀" id="button${faq.f_no}" class="ed-button"
								onclick="detail(n_no${faq.f_no}, 
												button${faq.f_no},
												title${faq.f_no},
												d_title${faq.f_no})">
						</td>
					</tr>
					<tr class="content" id="n_no${faq.f_no}" class="content">
						<td colspan="3">
							<div>
								${faq.f_content}
							</div>
	...
	<script>
		...
		xhr.open("get", "${path}/a06_notice/z01_fcList.jsp", true);
		xhr.onreadystatechange = function() {
			if(xhr.readyState == 4 && xhr.status == 200){
				var fc = JSON.parse(xhr.responseText);
				var faq_name = "${param.faq_name}";
				var faq_name_style = "";
				
				// FAQ_CODE	전체 선택(전체버튼을 주황색으로 변경)
				if(faq_name == "")
					faq_name_style = " style='color:#FF8533;'";
				else
					faq_name_style = "";
				var show = '<li><button'+faq_name_style+' name="faq_name" value="">전체</button></li>';
				
				// Ajax를 통해 버튼 생성 & 선택된 항목 주황색으로 변경 
				$(fc.fcList).each(function(idx, fc){
					if(faq_name == fc.fc_name) {
						faq_name_style = " style='color:#FF8533;'";
					}
					else {
						faq_name_style = "";
					} 
					show += '<li><button'+faq_name_style+' value="'+fc.fc_name+'" name=faq_name>'
						+fc.fc_name+'</button></li>';
				});
				$('.fqa_code_grop').html(show);
			}
		}
		xhr.send();
	</script>
